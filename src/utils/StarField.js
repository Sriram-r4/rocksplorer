export class StarField {
  ...
  setFade(distanceKm) {
    ...
    const t = Math.max(
      0,
      Math.min((distanceKm - minFadeStart) / (fullFadeAt - minFadeStart), 1)
    );
  ...
  update(isMoving = false, speedFactor = 1, horizontalDrift = 0) {
    ...
    this.stars.forEach((star) => {
      const flicker = Math.sin(now * star.twinkleSpeed) * 0.05;
      star.alpha = Math.max(
        0.3,
      ...
      star.radius =
        star.baseRadius *
        (isMoving ? 1.25 : 1) *
        (1 + Math.sin(now * star.twinkleSpeed) * 0.05);

      if (isMoving) {
        const moveY = (speedFactor / star.z) * 1.5;
        star.y += moveY;
        star.x += horizontalDrift * (1 / star.z);
  ...
  draw(isMoving = false) {
    ...
    this.stars.forEach((star) => {
      const hue = star.hue;
   if (isMoving) {
  // --- Warp streaks stretch dynamically based on current speed ---
  const spd = typeof speedFactor === "number" ? speedFactor : 0;
  ...
  const effectiveTrail = star.trailLength * boostMultiplier * (star.z * 0.4);

  const gradient = ctx.createLinearGradient(
    star.x,
    star.y - effectiveTrail,
    star.x,
    star.y
  );
  ...
  ctx.moveTo(star.x, star.y - effectiveTrail);
  ...
  ctx.stroke();
        ...
        gradient.addColorStop(
          0,
          `hsla(${hue}, 100%, 90%, ${star.alpha * 0.9})`
        );
